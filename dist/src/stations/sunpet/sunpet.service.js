...
let SunpetService = class SunpetService {
    ...
    async getPrice(id) {
        ...
        const cityName = constants_1.CITY_IDS[id]
            ...
            .replace(/ü/g, 'u')
            .trim();
        const url = sunpet_module_1.STATION.stationUrl.replace('{CITY_NAME}', cityName);
        let responses = [await this.httpService.axiosRef.get(url)];
        if (id === 34) {
            responses = [
                await this.httpService.axiosRef.get(sunpet_module_1.STATION.stationUrl.replace('{CITY_NAME}', 'istanbul-anadolu')),
                await this.httpService.axiosRef.get(sunpet_module_1.STATION.stationUrl.replace('{CITY_NAME}', 'istanbul-avrupa')),
            ];
        }
        responses.forEach((response) => {
            ...
            const fuelTableRows = $('body main div#fuel-prices-page section.fuel-prices-table-section div.container div.primary-table-wrapper table.primary-table tbody tr');
            fuelTableRows.each((index, element) => {
                const cells = $(element).find('td');
                const districtName = $(cells[sunpet_module_1.STATION.districtNameKey]).text().trim();
                const normalisedDistrictName = (0, districts_1.getDistrict)(id, districtName);
                if (!normalisedDistrictName)
                    return;
                const gasolinePrice = sunpet_module_1.STATION.hasGasoline
                    ? $(cells[sunpet_module_1.STATION.gasolineKey]).text().trim().replace(',', '.')
                    : null;
                const dieselPrice = sunpet_module_1.STATION.hasDiesel
                    ? $(cells[sunpet_module_1.STATION.dieselKey]).text().trim().replace(',', '.')
                    : null;
                const lpgPrice = sunpet_module_1.STATION.hasLpg
                    ? $(cells[sunpet_module_1.STATION.lpgKey]).text().trim().replace(',', '.')
                    : null;
                const fuel = {
                    cityName: constants_1.CITY_IDS[id],
                    districtName: normalisedDistrictName,
                    stationName: sunpet_module_1.STATION.displayName,
...
exports.SunpetService = SunpetService;
exports.SunpetService = SunpetService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [axios_1.HttpService,
        prisma_service_1.PrismaService])
], SunpetService);
//# sourceMappingURL=sunpet.service.js.map